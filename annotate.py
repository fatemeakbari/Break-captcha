...
ap.add_argument("-i", "--input",
                default="D:\project_Python\captcha_breaker\downloads",
help="path to input directory of images")
ap.add_argument("-a", "--annot",
                default="D:\project_Python\captcha_breaker\dataset",
...
for (i, imagePath) in enumerate(imagePaths):
    ...
    try:
        ...
        for c in cnts:
            ...
            key = cv2.waitKey(0)
            # if the ’‘’ key is pressed, then ignore the ch
            if key == ord("q"):
                print("[INFO] ignoring character")
                continue
            # grab the key that was pressed and construct the
